...
pub async fn main() {
    ...
    let socket = Arc::new(PeerConnectionEngine::new(host_addr));
    let addr = socket.local_addr;

    let handle = tokio::runtime::Handle::current();
    let server = Server::new_ssl(
        "0.0.0.0:3000",
        move |request| web_request(request, Arc::clone(&socket), &handle),
        certificate,
        private_key,
    )
...
fn web_request(request: &Request, engine: Arc<PeerConnectionEngine>, handle: &tokio::runtime::Handle) -> Response {
    ...
    handle.spawn(async move {
        ...
        {
            let mut rtc = peer.rtc.lock().unwrap();

            // Add the shared UDP socket as a host candidate
            let candidate = Candidate::host(engine.local_addr).expect("a host candidate");
            ...
            tx.send(Response::from_data("application/json", body)).unwrap();
        }
