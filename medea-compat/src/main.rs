...
fn web_request(
    ...
    handle.spawn(async move {
        let mut peer = engine.create_peer();
        peer.add_local_candidate(Candidate::host(engine.local_addr).unwrap()).await;
        ...
        tx.send(Response::from_data("application/json", body))
            .unwrap();

        connections.lock().await.push(peer);
    });
